...
3.  To update the existing CL on Gerrit without creating a new one:
    *   Squash the new commit into the previous one using `git reset --soft <base-commit>` and then committing again.
...
1.  Accept `OURS` (the baseline) during conflict resolution to get past the rebase conflict quickly.
