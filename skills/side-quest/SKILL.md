...
`/side-quest` solves this by maintaining an organic, multi-tiered visual map (`side_quests.md`) in session memory, keeping both human and agent anchored without adding friction or bloat.
...
### Session-Private Storage (`side_quests.md`)
Whenever `/side-quest` generates or updates the map, it writes **exclusively** to the agent's private session directory on disk:
`file://<appDataDir>/brain/<conversation-id>/side_quests.md`
...
Because the file is persisted to disk in the session folder, if the conversation undergoes **context compaction** (due to large log outputs or long turns), the agent can instantly re-read `side_quests.md` to recover exact task hierarchy without losing state.

---

## 🎯 Multi-Quest Hierarchy (`Main -> Sub -> Side`)

Rather than restricting the map to a single rigid goal, `/side-quest` supports **Multiple Main Quests** across two natural patterns:
...
### Mode B: Subagent Transcript Audit (`/side-quest rebuild`)
...
1. Spawn a background subagent using `invoke_subagent` (or equivalent subagent tool) with the following configuration:
   - `TypeName`: `"self"`
   - `Role`: `"Side-Quest Log Auditor"`
   - `Prompt`: Use this exact self-contained prompt (filling in your real `<conversation-id>` and `<appDataDir>`):
     ```
     You are a background Side-Quest Log Auditor. Your sole job is to inspect the full conversation transcript for conversation ID: <conversation-id> at `<appDataDir>/brain/<conversation-id>/.system_generated/logs/transcript.jsonl` and build/rebuild the visual hierarchy map.
     ...
     3. Write the finalized markdown table using `write_to_file` (with `Overwrite: true`) to:
        `<appDataDir>/brain/<conversation-id>/side_quests.md`
     4. When done, call `send_message` to your parent agent confirming that `side_quests.md` has been successfully built/rebuilt.
     ```
...
While `side_quests.md` handles in-session digressions cleanly, some side quests cannot be resolved in one sitting (e.g., waiting on external team reviews, security approvals, or multi-day refactors).

For items marked **`[Parked / Tracked for Later]`** in `side_quests.md`, the skill bridges seamlessly into your existing persistence tools:
...
Once `side_quests.md` exists, the agent adopts a helpful, low-friction discipline:
- **No Heavy Pushback:** When the user pivots across files or topics, acknowledge it smoothly: *"Oh, we're going off on a side-quest, that's completely fine."*
- **Gentle Triage Prompts:** When a new unexpected blocker or rabbit hole emerges (e.g., a broken build or linter warning), ask:
  > *"We're diving into a side quest to fix this dependency—that's totally fine. Should we tackle it right now, or track it in our artifact?"*
...
- **Chapter-Break Awareness:** When a major PR is pushed or a task completes and the user introduces a new topic, recognize the chapter break:
  > *"Looks like we finished Main Quest 1! Should we open a new Main Quest in our map for this new topic, or is this just a quick side quest?"*

---

## 📄 Template: `side_quests.md`

```markdown
# 🗺️ Conversation Map & Side Quests
